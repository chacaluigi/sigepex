...
const actualizarEstadoPago = async (req, res = response) => {
  try {
    ...
    if (pago.estado === "PENDIENTE" || pago.estado === "INCOMPLETO") {
      pago.importe = pago.concepto.reduce(
        (acum, item) => acum + item.precio,
        0.0
      );
      pago.observaciones = "Pago cancelado por el usuario";
      pago.estado = "CANCELADO";
    }

    await pago.save();

    const pagoActualizada = await Pago.findOne({ _id: pago._id })
